...
public class Query2_all_days {
    private static final Map<String, Map<Integer, VaultInfo>> globalVaultData = new HashMap<>();
    private static final Map<Integer, VaultInfo> specialDayVaultData = new HashMap<>();
    ...
    public static void main(String[] args) throws Exception {
        ...
        DataStream<String> stream = env.addSource(consumer)
                ...
                .map(value -> value)
                .filter(value -> {
                    // Parse JSON and filter by failure
        ...
        // Add a custom sink to ensure all data is processed at the end
        stream.addSink(new RichSinkFunction<String>() {
            @Override
            public void close() throws Exception {
                System.out.println("Closing the sink and writing remaining data...");
                // Force final processing of remaining data
                writeOutputForDates(globalVaultData, System.currentTimeMillis(), 0, 0);

                // Special handling for 2023-04-23
                writeSpecialDayOutput(specialDayVaultData, System.currentTimeMillis(), 0, 0);
            }
        });

        env.execute("Query_2_all_days");
    }
    ...
    public static class CalculateStatistics implements AllWindowFunction<String, String, TimeWindow> {
        ...
        public void apply(TimeWindow window, Iterable<String> values, Collector<String> out) throws Exception {
            ...
            for (String value : values) {
                ...
                if ("2023-04-23".equals(date)) {
                    VaultInfo vaultInfo = specialDayVaultData.getOrDefault(vaultId, new VaultInfo());
                    vaultInfo.incrementFailures();
                    vaultInfo.addModelSerial(model, serialNumber);
                    specialDayVaultData.put(vaultId, vaultInfo);
                } else {
            ...
            writeOutputForDates(globalVaultData, windowStart, latency, throughput);

            // Special handling for 2023-04-23
            writeSpecialDayOutput(specialDayVaultData, windowStart, latency, throughput);
        }
    }

    private static void writeOutputForDates(Map<String, Map<Integer, VaultInfo>> globalVaultData, long timestamp, long latency, double throughput) {
        for (String date : globalVaultData.keySet()) {
            ...
            csvBatch.append(timestamp).append(",")
            ...
            File file = new File("/opt/flink/Results/Query2/Query2_all_days/output_" + date + ".csv");
            ...
            try (FileWriter writer = new FileWriter(file, true)) {
                ...
                System.out.println("Wrote data for date: " + date);
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
    ...
    private static void writeSpecialDayOutput(Map<Integer, VaultInfo> specialDayVaultData, long timestamp, long latency, double throughput) {
        // Get top 10 vaults by number of failures for the special day
        List<Map.Entry<Integer, VaultInfo>> topVaults = specialDayVaultData.entrySet().stream()
                .sorted((e1, e2) -> Integer.compare(e2.getValue().getFailures(), e1.getValue().getFailures()))
                .limit(10)
                .collect(Collectors.toList());
        ...
        // Write the batch to a file
        File file = new File("/opt/flink/Results/Query2/Query2_all_days/output_2023-04-23.csv");
        boolean fileExists = file.exists() && file.length() != 0;
        try (FileWriter writer = new FileWriter(file, true)) {
            if (!fileExists) {
                writer.write("window,latency,throughput,vault_id1,failures1,model_serial_list1,...,vault_id10,failures10,model_serial_list10\n");
            ...
            writer.write(csvBatch.toString());
            System.out.println("Wrote data for special day: 2023-04-23");
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
